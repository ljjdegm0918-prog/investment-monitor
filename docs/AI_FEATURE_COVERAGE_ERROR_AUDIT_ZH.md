...
- CSE filings 在本地缓存为空时会尝试刷新 TSX、TSXV 和 CSE 证券目录。本次实跑中，CSE 官方目录出现一条状态为 `Delisted`、但缺少 `delistedDate` 的记录；解析器按数据契约失败关闭并跳过整块 CSE 数据，避免猜测退市日期。网络、DNS 或同类上游字段错误都属于运行时外部端点问题，不应当当作空结果。
- HKEXnews 本次实跑返回了与请求不一致的页面语言，连接器拒绝把该响应当作正确公告；这是上游响应契约异常。
- Substack 本次实跑包含 TLS 握手超时、协议提前结束和一个订阅源 HTTP 404。其它订阅源仍可独立工作，因此应保留逐源失败详情，不把整个社区来源伪装成正常。
