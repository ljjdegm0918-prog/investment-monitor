...
def feed_item(
    ...
    external_id: str,
    *,
    title: str = "BlackRock Global Allocation Fund updates factsheet",
    ticker: str = "LU0171254561",
    market: str = "emf",
...
class EmfDedupeKeyTests(unittest.TestCase):
    def test_emf_news_key_is_non_empty(self) -> None:
        item = feed_item("google_news_emf", "g-1")

        self.assertIsNotNone(dedupe_key(item))
        self.assertTrue(dedupe_key(item).startswith("emf-news:"))
        self.assertIn("LU0171254561|2026-08-05|", dedupe_key(item))

    def test_same_isin_day_title_shares_key(self) -> None:
        first = feed_item("google_news_emf", "g-1")
        second = feed_item("google_news_emf", "g-2")
    ...
    def test_different_title_does_not_fold(self) -> None:
        first = feed_item("google_news_emf", "g-1", title="Fund factsheet")
        second = feed_item("google_news_emf", "g-2", title="Fund inflows")

        self.assertNotEqual(dedupe_key(first), dedupe_key(second))

    def test_different_isin_does_not_fold(self) -> None:
        first = feed_item("google_news_emf", "g-1", ticker="LU0171254561")
        second = feed_item("google_news_emf", "g-2", ticker="LU1290091628")

        self.assertNotEqual(dedupe_key(first), dedupe_key(second))

    def test_luxembourg_day_uses_fund_timezone(self) -> None:
        # 22:30 UTC on 2026-08-09 is 00:30 CEST on 2026-08-10 (same
        # Luxembourg day as 21:00 UTC on 2026-08-10); 22:30 UTC on
        # 2026-08-10 is 00:30 CEST on 2026-08-11.
        late_utc = feed_item(
            "google_news_emf",
            "g-1",
    ...
    def test_emf_filing_never_gets_a_key(self) -> None:
        item = feed_item(
            "emf_disclosure",
            "f-1",
    ...
    def test_non_emf_markets_still_use_their_own_rules(self) -> None:
        cxe = feed_item(
            "google_news_cxe",
            "cxe-1",
            market="cxe",
        ...
        self.assertTrue(dedupe_key(cxe).startswith("cxe-news:"))
        self.assertTrue(dedupe_key(de).startswith("de-news:"))


class EmfAnnotateFeedItemsTests(unittest.TestCase):
    def test_same_key_keeps_both_rows_annotated(self) -> None:
        ...
        self.assertEqual(annotated[0]["also_seen_on"], ["google_news_emf"])
        self.assertEqual(
...
class WebRepositoryEmfDedupeTests(unittest.TestCase):
    def test_display_annotates_emf_news_while_raw_keeps_rows(self) -> None:
        with TemporaryDirectory() as temporary_directory:
            database_path = Path(temporary_directory) / "web.sqlite3"
            items = SQLiteInformationRepository(database_path)
            repository = WebRepository(
                database_path,
                allowed_sources=("google_news_emf",),
                known_sources=(
                    SourceConfig(
                        "google_news_emf",
                        "Google News (EMF)",
                ...
                implemented_sources=("google_news_emf",),
            )
            ...
            items.save([
                InformationItem(
                    ...
                    title="BlackRock Global Allocation Fund updates factsheet",
                    document_type="news",
            ...
            self.assertEqual(labels, {("Google News (EMF)",)})
