...
class GoogleSeClientTests(unittest.TestCase):
    def test_parses_rss_and_builds_sweden_query(self) -> None:
        from investment_monitor.sources.se_news.google.client import (
            GoogleSeNewsClient,
            _parse_rss,
        )

        body = (FIXTURES / "google_se_ericb.xml").read_bytes()
        records = _parse_rss(
            body,
        ...
        self.assertIn("q=ERIC-B.ST", opener.requested[0])
        self.assertIn("hl=sv", opener.requested[0])
        self.assertIn("gl=SE", opener.requested[0])
        self.assertIn("ceid=SE:sv", opener.requested[0])

    def test_malformed_feed_raises_data_error(self) -> None:
        from investment_monitor.sources.se_news.google.client import _parse_rss

        with self.assertRaises(GoogleSeNewsDataError):
            _parse_rss(
...
class GoogleSeConnectorTests(unittest.TestCase):
    ...
    def make_connector(self, symbol_for=None):
        ...
        opener = FakeOpener((FIXTURES / "google_se_ericb.xml").read_bytes())
        connector = GoogleSeNewsConnector(
            client=GoogleSeNewsClient(
                opener=opener,
                requests_per_second=1000,
            ),
    ...
    def test_se_maps_news_with_canonical_ticker(self) -> None:
        connector, opener = self.make_connector()
        ...
        first = items[0]
        self.assertEqual(first.source, "google_news_se")
        self.assertEqual(first.source_type, "news")
        self.assertEqual(first.tickers, ("ERIC-B",))
        self.assertEqual(first.market, "se")
        self.assertEqual(first.raw_metadata["provider"], "google_news_rss")
        self.assertEqual(first.raw_metadata["langs"], "sv")
        self.assertIn("q=ERIC-B.ST", opener.requested[0])
        for item in items:
    ...
    def test_symbol_for_injection_is_used(self) -> None:
        ...
        self.assertIn("q=ERIC-B.TEST", opener.requested[0])
    ...
    def test_single_ticker_failure_raises_and_records_error(self) -> None:
        ...
        def failing_opener(request, timeout=None):
            raise GoogleSeNewsRequestError("google blocked")
        ...
        with self.assertRaises(GoogleSeNewsRequestError):
            connector.collect(self.request(("ERIC-B",), {"ERIC-B": "se"}))

        self.assertEqual(len(connector.last_errors), 1)
        self.assertEqual(connector.last_errors[0][0], "ERIC-B")

    def test_registry_registers_google_se_without_secret_field(self) -> None:
        registry = create_default_registry()

        self.assertIsNotNone(registry.factory_for("google_news_se"))
        self.assertEqual(registry.secret_fields_for("google_news_se"), ())
