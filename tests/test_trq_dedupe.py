...
class TrqDedupeKeyTests(unittest.TestCase):
    def test_trq_news_key_is_non_empty(self) -> None:
        item = feed_item("google_news_trq", "g-1")

        self.assertIsNotNone(dedupe_key(item))
        self.assertTrue(dedupe_key(item).startswith("trq-news:"))
        self.assertIn("AZN|2026-08-05|", dedupe_key(item))

    def test_same_ticker_day_title_shares_key(self) -> None:
        first = feed_item("google_news_trq", "g-1")
        second = feed_item("google_news_trq", "g-2")
    ...
    def test_different_title_does_not_fold(self) -> None:
        first = feed_item("google_news_trq", "g-1", title="Oncology data")
        second = feed_item("google_news_trq", "g-2", title="Pharma rally")

        self.assertNotEqual(dedupe_key(first), dedupe_key(second))

    def test_different_ticker_does_not_fold(self) -> None:
        first = feed_item("google_news_trq", "g-1", ticker="AZN")
        second = feed_item("google_news_trq", "g-2", ticker="SHEL")
    ...
    def test_trq_filing_never_gets_a_key(self) -> None:
        item = feed_item(
            "trq_disclosure",
            "f-1",
    ...
    def test_non_trq_markets_still_use_their_own_rules(self) -> None:
        cxe = feed_item(
            "google_news_cxe",
            "cxe-1",
            market="cxe",
        ...
        emf = feed_item(
            ...
            "emf-1",
            market="emf",
            title="BlackRock factsheet",
        )

        self.assertTrue(dedupe_key(cxe).startswith("cxe-news:"))
        self.assertTrue(dedupe_key(emf).startswith("emf-news:"))


class TrqAnnotateFeedItemsTests(unittest.TestCase):
    def test_same_key_keeps_both_rows_annotated(self) -> None:
        ...
        self.assertEqual(annotated[0]["also_seen_on"], ["google_news_trq"])
        self.assertEqual(
...
class WebRepositoryTrqDedupeTests(unittest.TestCase):
    def test_display_annotates_trq_news_while_raw_keeps_rows(self) -> None:
        with TemporaryDirectory() as temporary_directory:
            database_path = Path(temporary_directory) / "web.sqlite3"
            items = SQLiteInformationRepository(database_path)
            repository = WebRepository(
                database_path,
                allowed_sources=("google_news_trq",),
                known_sources=(
                    SourceConfig(
                        "google_news_trq",
                        "Google News (TRQ)",
                ...
                implemented_sources=("google_news_trq",),
            )
            ...
            self.assertEqual(labels, {("Google News (TRQ)",)})
