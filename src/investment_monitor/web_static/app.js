...
const MARKET_HINTS_EN = {
  ...
  se: "SE market (Nasdaq Stockholm / First North Sweden): root tickers strip .ST/.STO/.OMX; share-class suffixes like -B/-A are kept (ERIC-B stays ERIC-B); Swedish ISINs kept as-is. Official disclosure is wired through Nasdaq Nordic Company News for both Stockholm Main Market and First North Sweden. The connector resolves the ticker with Nasdaq's official share directory, follows the declared result count to completion, preserves the official raw JSON, and fails closed on ambiguous identity or pagination mismatch. FI publiceringsklient remains excluded because it is an insider-transactions search, not a replacement issuer-announcement stream. News: Yahoo Finance SE + Google News SE (key-free RSS; .ST at request time; loosely related possible). SE companies are added as unmapped for SEC purposes. Finnhub is US-only and never queried for SE.",
  ...
  eux: "EUX market (Eurex Core, derivatives exchange): futures/options product codes are kept uppercased (FDAX stays FDAX); .EUX suffixes stripped at add time; ISINs kept as-is. Products are added as unmapped (no SEC mapping). Official exchange notices are wired through the complete Eurex Circulars & Newsflashes archive. The connector follows Eurex's own next-page links until the requested date window is exhausted and fails closed on structure or pagination errors. These are exchange-wide notices, not issuer OAM filings for an underlying stock. Universe: official Eurex product list CSV backfills product name/ISIN/group. News: Google News (EUX) remains a separate, non-official discovery feed. The Display Value Bundle is a market-data product, not a filing source.",
};
...
const MARKET_HINTS_ZH = {
  ...
  se: "SE 市场（Nasdaq Stockholm / First North Sweden）：根代码去除 .ST/.STO/.OMX；份额类后缀如 -B/-A 保留（ERIC-B 保持 ERIC-B）；瑞典 ISIN 保持原样。官方披露已接入 Nasdaq Nordic Company News，同时覆盖 Stockholm Main Market 和 First North Sweden。connector 使用 Nasdaq 官方证券目录解析 ticker，按接口声明总数分页到底，保存官方原始 JSON；发行人映射含糊或分页数量不符时会明确失败，不会伪装完整。FI publiceringsklient 仍不作为替代，因为它是内幕交易查询而非发行人公告流。新闻：Yahoo Finance SE + Google News SE（免 key RSS；可能松散相关）。SE 公司仅在 SEC 映射意义上保持未映射；Finnhub 不查询 SE。",
  ...
  eux: "EUX 市场（Eurex Core，衍生品交易所）：期货/期权产品代码保持大写（FDAX 保持 FDAX）；添加时去除 .EUX 后缀；ISIN 保持原样。官方交易所通知已接入完整的 Eurex Circulars & Newsflashes 档案。connector 沿 Eurex 官方下一页链接抓取，直到越过请求日期范围；页面结构或分页异常时明确失败。这些是交易所级通知，不是某只标的股票发行人的 OAM 公告。宇宙仍由 Eurex 官方产品列表 CSV 回填。Google News (EUX) 是独立的非官方发现来源。Display Value Bundle 是行情数据产品，不是 filing 来源。",
};
