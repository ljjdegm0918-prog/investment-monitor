...
const MARKET_HINTS = {
  ...
  se: "SE market (Nasdaq Stockholm / First North Sweden): root tickers strip .ST/.STO/.OMX; share-class suffixes like -B/-A are kept (ERIC-B stays ERIC-B); Swedish ISINs kept as-is. Disclosure is NOT wired (SE-1 spike: FI publiceringsklient is insider-transactions only; Nasdaq Nordic company news is a Drupal SPA with no public JSON; old OMX disclosure search HTTP 500; EQS empty for sampled Swedish ISINs; no paid Nasdaq data products). Universe: boundary stub (SE-2 spike B2: Nasdaq Stockholm/First North directories are JS screener SPAs without a reachable public JSON route; refresh raises SeUniverseError; no OMXS30 seed). News connectors are not wired yet (SE-3 through SE-5 in progress); SE companies are added as unmapped. Finnhub is US-only and never queried for SE.",
};
