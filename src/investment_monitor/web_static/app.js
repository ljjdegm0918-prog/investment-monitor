...
function marketLabel(market) { return ({ us: "US", cn: "CN", hk: "HK", kr: "KR", uk: "UK", tw: "TW", ca: "CA", au: "AU", unknown: "Market unknown" })[market] || esc(market || "Unknown"); }

const MARKET_HINTS = {
  au: "AU market: Australian symbols normalize to root tickers (.AX/.ASX suffixes are stripped; board goes into exchange later). ASX company announcements are connected via the key-free ASX site research API (undocumented; may change without notice; latest 5 announcements per company only; no PDF deep links; re-verified AU-4 — no second free disclosure source found). Universe: the ASX company directory is connected via the same key-free API and backfills names/board (ASX) for add-company; not IBKR-complete; never enters the feed. News: Yahoo Finance AU and Google News (AU) are connected via key-free public RSS (may be loosely related; may break without notice), with display-only 'Also seen on' annotations. AU companies are added as unmapped. Finnhub is US-only and never queried for AU.",
  ca: "CA market: Canadian symbols normalize to root tickers (.TO/.TSX/.V/.TSXV/.CN/.NE/.NEO suffixes are stripped; board goes into exchange later). Universe: TSX and TSXV company directories are connected via key-free official TMX JSON and backfill names/board for add-company; CSE and NEO are not wired (TLS handshake failure / origin timeout). News: Yahoo Finance CA and Google News (CA) are connected via key-free public RSS (may be loosely related; may break without notice), with display-only 'Also seen on' annotations. Disclosure is deliberately NOT wired yet (CA-1 spike, re-verified CA-4): SEDAR+ has no official public API and its real search endpoints return 403 to plain HTTP clients; no stable key-free alternative was found. CA companies are added as unmapped. Finnhub is US-only and never queried for CA.",
  ...
  tw: "TWSE (listed) and TPEx (OTC) OpenAPI material-information (重大訊息) are connected (key-free; not a paid MOPS push). 興櫃 (Emerging) disclosure is not wired yet. Yahoo Finance TW and Google News (TW) are connected via key-free public RSS (may be loosely related; may break without notice). A TWSE/TPEx universe cache can backfill company names and board for add-company. Finnhub is US-only and never queried for TW.",
