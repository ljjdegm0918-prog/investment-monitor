...
const MARKET_HINTS = {
  ...
  cxe: "CXE market (Cboe Europe equities, CXE/BXE books): first Alternative European Equities venue only - NOT the whole package (Turquoise and other MTFs are deferred). Cboe symbols are kept uppercased (AZNl -> AZNL); .CXE/.BXE suffixes stripped at add time; ISINs kept as-is. Companies are added as unmapped (no SEC mapping). Disclosure is NOT wired (AEE-1 A3: Cboe Europe is an MTF without an independent issuer OAM; venue symbol/trade pages are not issuer disclosures). Universe: official Cboe Europe Symbol Data CSVs for CXE+BXE backfill names/exchange/venue (~5.3k + ~6.5k symbols live 2026-08-10; no ISIN column). News: Google News (CXE) via key-free RSS (queries company name from universe, else Cboe symbol; no Yahoo suffix exists for Cboe Europe, so no yahoo_cxe). Finnhub is US-only and never queried for CXE.",
};
