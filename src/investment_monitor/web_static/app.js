...
const MARKET_HINTS = {
  au: "AU market: Australian symbols normalize to root tickers (.AX/.ASX suffixes are stripped; board goes into exchange later). ASX company announcements are connected via the key-free ASX site research API (undocumented; may change without notice; latest 5 announcements per company only). AU universe and news connectors are not wired yet (AU-2/AU-3); AU companies are added as unmapped. Finnhub is US-only and never queried for AU.",
