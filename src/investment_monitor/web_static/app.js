...
const MARKET_HINTS = {
  ...
  sg: "SG market (SGX): root tickers strip .SI/.SG; Singapore ISINs kept as-is; SGX codes vary in length (no fixed width). Disclosure is NOT wired (SG-1 spike: SGX announcements are a JS SPA; api.sgx.com returns 403; legacy infopub SGXNet JSON retired; links.sgx.com has deep links only; no paid SGX DataLink). Universe and news connectors are not wired yet (SG-2 through SG-5 in progress); SG companies are added as unmapped. Finnhub is US-only and never queried for SG.",
};
