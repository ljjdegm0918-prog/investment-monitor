...
const MARKET_HINTS = {
  ...
  emf: "EMF market (European Mutual Funds / UCITS): ISIN-first identifiers (12-character fund ISINs kept as-is; .F/.MF fund-data suffixes stripped at add time). Funds are added as unmapped (no SEC mapping). This is NOT the German ETF (market=de) or Cboe Europe (market=cxe) universe, and not Eurex derivatives. Disclosure is NOT wired (EMF-1 A3: ESMA exposes only AIFMD fund reports without ISINs and no UCITS register; KIID/PRIIPs live on manager sites; no stock OAM re-mapped). Universe: boundary stub (EMF-2 B2: no stable free ISIN-bearing European fund directory; refresh raises EmfUniverseError; no fund seed). News: Google News (EMF) via key-free RSS (fund name from a manually placed cache, else typed ISIN - usually sparse; no Yahoo suffix exists for European funds, so no yahoo_emf). Finnhub is US-only and never queried for EMF.",
};
