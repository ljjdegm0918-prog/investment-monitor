...
const MARKET_HINTS = {
  ca: "CA market: Canadian symbols normalize to root tickers (.TO/.TSX/.V/.TSXV/.CN/.NE/.NEO suffixes are stripped; board goes into exchange later). Universe: TSX and TSXV company directories are connected via key-free official TMX JSON and backfill names/board for add-company; CSE and NEO are not wired (TLS handshake failure / origin timeout). Disclosure is deliberately NOT wired yet: SEDAR+ has no official public API and its Radware edge blocks plain HTTP clients (403 on every path), and no stable key-free alternative was found in the CA-1 spike. News connectors are planned (CA-3); CA companies are added as unmapped. Finnhub is US-only and never queried for CA.",
