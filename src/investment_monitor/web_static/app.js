...
const MARKET_HINTS = {
  ...
  ch: "CH market (SIX Swiss Exchange): root tickers strip .SW/.SWX/.S; Swiss ISINs kept as-is. Disclosure: EQS News (CH) via key-free JSON by Swiss ISIN (unofficial; partial coverage - Roche/UBS yes, some ISINs empty; NOT SIX/FINMA official; SIX official notices are a JS SPA and equity-issuer news is paid Exfeed). Needs ISIN from the CH universe cache (CH-2) or a typed Swiss ISIN. Universe and news connectors are not wired yet (CH-2 through CH-5 in progress); CH companies are added as unmapped. Finnhub is US-only and never queried for CH.",
};
