...
const MARKET_HINTS = {
  ...
  de: "DE market (Xetra / Deutsche Börse Cash Market): root tickers strip .DE/.XETRA/.XE/.F; German ISINs kept as-is. EQS News (DGAP) disclosure via key-free JSON (needs ISIN from universe or typed ISIN; EQS returns no records for sampled Xetra ETF ISINs - ETF disclosure not deepened). Universe cache now includes Xetra CS + ETF + ETN + ETC (instrument_type stored; live ~1,422 CS / 3,082 ETF / 385 ETN / 205 ETC) and backfills name/board/ISIN. News: Yahoo DE + Google News DE shared by stocks and ETFs (.DE at request time; ETF feeds may be empty/loosely related). Unternehmensregister/BaFin HTML not wired. Companies stay unmapped. Finnhub is US-only.",
