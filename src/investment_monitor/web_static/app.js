...
const MARKET_HINTS = {
  ...
  eux: "EUX market (Eurex Core, derivatives exchange): futures/options product codes are kept uppercased (FDAX stays FDAX); .EUX suffixes stripped at add time; ISINs kept as-is. Products are added as unmapped (no SEC mapping). This is NOT Xetra stocks/ETFs (market=de), NOT AEE (cxe/trq), NOT AQSE/Mutual Funds/Display Bundle. Disclosure is NOT wired (EUX-1 A3: Eurex circulars are a JS search surface without per-product rows/JSON; derivatives have no issuer OAM; no stock OAM re-mapped). Universe: official Eurex product list CSV backfills product name/ISIN/group (~2,997 products live 2026-08-11; product-level; never in feed; TLS intermittent). News: Google News (EUX) via key-free RSS (product name from universe, else Eurex product code; hl=de&gl=DE; no Yahoo suffix exists for Eurex derivatives, so no yahoo_eux). Finnhub is US-only and never queried for EUX.",
};
