...
const MARKET_HINTS = {
  ...
  trq: "TRQ market (Turquoise, LSEG MTF): second Alternative European Equities venue after Cboe Europe (market=cxe) - NOT the whole package and NOT AQSE/LSE/Eurex. Common symbols are kept uppercased (AZN stays AZN); .TRQ/.TRQX/.TQEX suffixes stripped at add time; ISINs kept as-is. Companies are added as unmapped (no SEC mapping). Disclosure is NOT wired (TRQ-1 A3: MTF without issuer OAM; re-test 2026-08-11: turquoise.com parked, turquoise.eu is an unrelated company, tradeturquoise.com redirects to a JS-only LSE SPA, old TRQX/TQEX reference-file CSVs 404; no stock OAM re-mapped). Universe: boundary stub (TRQ-2 B2: no stable free Turquoise directory; refresh raises TrqUniverseError; no CXE CSV reuse; no seed). News: Google News (TRQ) via key-free RSS (company name from a manually placed cache, else Turquoise symbol; no Yahoo suffix exists for Turquoise, so no yahoo_trq). Finnhub is US-only and never queried for TRQ.",
};
