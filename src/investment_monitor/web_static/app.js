...
const MARKET_HINTS = {
  ...
  hk: "HKEXnews announcement search is connected (not an official HKEX API/IIS feed; page may change without notice). HK symbols without a stock match are still added as unmapped.",
...
function batchResult(result) {
  const sections = [];
  if (result.added.length) sections.push(`<strong>Added:</strong> ${result.added.map(item => `${item.ticker} ${marketLabel(item.market)} (${esc(item.name)}, ${item.market === "kr" && item.cik ? `Corp code ${esc(item.cik)}` : item.market === "uk" && item.cik ? `Company no ${esc(item.cik)}` : item.market === "hk" && item.cik ? `HKEX id ${esc(item.cik)}` : item.cik ? `CIK ${esc(item.cik)}` : "Unmapped"}${item.mapping_status === "unmapped" ? ", unmapped for SEC" : ""})`).join(", ")}`);
