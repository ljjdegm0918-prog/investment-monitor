...
const MARKET_HINTS = {
  ...
  be: "BE market (Euronext Brussels): root tickers strip .BR/.BRU/.EBR; Belgian ISINs kept as-is. Disclosure: FSMA STORI (official key-free Belgian central storage of regulated information) is wired and matches by BE ISIN or company name - mnemonic tickers need the BE universe (BE-2) to supply an ISIN or a BE ISIN typed directly; tickers without an identity are skipped honestly. The BE universe (BE-2) and BE news (BE-3) are not wired yet. BE companies are added as unmapped. Finnhub is US-only and never queried for BE.",
