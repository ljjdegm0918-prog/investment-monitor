...
const MARKET_HINTS = {
  ...
  sg: "SG market (SGX): root tickers strip .SI/.SG; Singapore ISINs kept as-is; SGX codes vary in length (no fixed width). Disclosure is NOT wired (SG-1 spike: SGX announcements are a JS SPA; api.sgx.com returns 403; legacy infopub SGXNet JSON retired; links.sgx.com has deep links only; no paid SGX DataLink). The SG universe is a boundary stub (no stable free SGX directory; refresh raises; cache shape reserved). News connectors are not wired yet (SG-3 through SG-5 in progress); SG companies are added as unmapped. Finnhub is US-only and never queried for SG.",
};
...
function regionForMarket(market) { return ({us:"United States",jp:"Japan",hk:"Hong Kong",cn:"China",kr:"Korea",uk:"United Kingdom",tw:"Taiwan",ca:"Canada",au:"Australia",fr:"France",de:"Germany",nl:"Netherlands",it:"Italy",es:"Spain",sg:"Singapore"})[market] || "Unavailable"; }
