"""Google News (PL) connector for market=pl companies."""
...
class GooglePlNewsConnector:
    """Collect Google News RSS items for market=pl companies."""

    name = "google_news_pl"
    provider = "Google News (PL)"
    max_lookback_days = MAX_LOOKBACK_DAYS

    def __init__(
        self,
        client: Optional[GooglePlNewsClient] = None,
        symbol_for: Optional[Callable[[str], str]] = None,
    ) -> None:
        self._client = client or GooglePlNewsClient.from_environment()
    ...
    def collect(self, request: CollectionRequest) -> List[InformationItem]:
        ...
        for ticker in request.tickers:
            ...
            try:
                ...
                LOGGER.warning(
                    "google_news_pl ticker=%s status=failure error=%s",
                    ticker,
                    message,
                )
        self._last_errors = tuple(failures)
        if len(request.tickers) == 1 and failures:
            raise GooglePlNewsRequestError(failures[0][1])
        return items
