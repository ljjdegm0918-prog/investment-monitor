"""TPEx OpenAPI material-information connector for market=tw OTC companies."""
...
from ..twse_material.client import MOPS_QUERY_URL, normalize_tw_ticker
from .client import (
    TpexMaterialClient,
...
class TpexMaterialConnector:
    """Collect TPEx OTC material announcements for market=tw companies."""

    name = "tpex_material"
    provider = "TPEx OpenAPI (material)"
    max_lookback_days = MAX_LOOKBACK_DAYS
    # Key-free: no secret_fields and no configuration_error.

    def __init__(self, client: Optional[TpexMaterialClient] = None) -> None:
        self._client = client or TpexMaterialClient.from_environment()
    ...
    def collect(self, request: CollectionRequest) -> List[InformationItem]:
        ...
        try:
            ...
            self._last_errors = tuple(
                (ticker, message) for ticker in tw_tickers
            )
            LOGGER.warning(
                "tpex_material status=failure error=%s",
                message,
            )
            if len(request.tickers) == 1:
                raise TpexMaterialRequestError(message)
            return []
