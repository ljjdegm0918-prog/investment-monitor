"""Google News (IT) connector for market=it companies."""
...
class GoogleItNewsConnector:
    """Collect Google News RSS items for market=it companies."""

    name = "google_news_it"
    provider = "Google News (IT)"
    max_lookback_days = MAX_LOOKBACK_DAYS

    def __init__(
        self,
        client: Optional[GoogleItNewsClient] = None,
        symbol_for: Optional[Callable[[str], str]] = None,
    ) -> None:
        self._client = client or GoogleItNewsClient.from_environment()
    ...
    def collect(self, request: CollectionRequest) -> List[InformationItem]:
        ...
        for ticker in request.tickers:
            ...
            try:
                ...
                LOGGER.warning(
                    "google_news_it ticker=%s status=failure error=%s",
                    ticker,
                    message,
                )
        self._last_errors = tuple(failures)
        if len(request.tickers) == 1 and failures:
            raise GoogleItNewsRequestError(failures[0][1])
        return items
