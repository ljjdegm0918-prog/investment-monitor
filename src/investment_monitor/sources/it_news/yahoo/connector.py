"""Yahoo Finance IT news connector for market=it companies."""

from __future__ import annotations

import logging
from datetime import datetime, timezone
from typing import Any, Callable, Dict, List, Mapping, Optional, Tuple

from ....models import CollectionRequest, InformationItem, MARKET_IT
from ....web_repository import normalize_it_ticker
from ..symbols import it_yahoo_symbol
from .client import (
    YahooItNewsClient,
    YahooItNewsRequestError,
)

LOGGER = logging.getLogger(__name__)

MAX_LOOKBACK_DAYS = 30


class YahooItNewsConnector:
    """Collect Yahoo Finance Italy stock news for market=it companies."""

    name = "yahoo_it"
    provider = "Yahoo Finance IT"
    max_lookback_days = MAX_LOOKBACK_DAYS

    def __init__(
        self,
        client: Optional[YahooItNewsClient] = None,
        symbol_for: Optional[Callable[[str], str]] = None,
    ) -> None:
        self._client = client or YahooItNewsClient.from_environment()
        self._symbol_for = symbol_for or _default_symbol_for
        self._last_errors: Tuple[Tuple[str, str], ...] = ()

    @property
    def last_errors(self) -> Tuple[Tuple[str, str], ...]:
        return self._last_errors

    def collect(self, request: CollectionRequest) -> List[InformationItem]:
        items: List[InformationItem] = []
        failures: List[Tuple[str, str]] = []
        collected_at = datetime.now(timezone.utc)
        for ticker in request.tickers:
            market = request.market_for(ticker)
            if market != MARKET_IT:
                LOGGER.info(
                    "yahoo_it ticker=%s market=%s skipped not_it_market",
                    ticker,
                    market,
                )
                continue
            code = normalize_it_ticker(ticker)
            symbol = self._symbol_for(code)
            try:
                it_records = self._client.fetch_news(
                    symbol,
                    request.start_date,
                    request.end_date,
                    lang="it-IT",
                )
                en_records = self._client.fetch_news(
                    symbol,
                    request.start_date,
                    request.end_date,
                    lang="en-US",
                )
                items.extend(
                    _map_news(
                        it_records,
                        en_records,
                        code=code,
                        collected_at=collected_at,
                    )
                )
            except Exception as error:
                message = str(error) or error.__class__.__name__
                failures.append((ticker, message))
                LOGGER.warning(
                    "yahoo_it ticker=%s status=failure error=%s",
                    ticker,
                    message,
                )
        self._last_errors = tuple(failures)
        if len(request.tickers) == 1 and failures:
            raise YahooItNewsRequestError(failures[0][1])
        return items


def _default_symbol_for(ticker: str) -> str:
    """Request-time symbol: canonical IT root plus the .MI suffix."""
    return it_yahoo_symbol(ticker)


def _map_news(
    it_records: List[Mapping[str, Any]],
    en_records: List[Mapping[str, Any]],
    *,
    code: str,
    collected_at: datetime,
) -> List[InformationItem]:
    merged: Dict[str, Dict[str, Optional[Mapping[str, Any]]]] = {}
    for record in it_records:
        merged[str(record["external_id"])] = {"it": record, "en": None}
    for record in en_records:
        key = str(record["external_id"])
        if key in merged:
            merged[key]["en"] = record
        else:
            merged[key] = {"it": None, "en": record}

    items: List[InformationItem] = []
    for key, pair in merged.items():
        it = pair["it"]
        en = pair["en"]
        record = en or it
        if record is None:
            continue
        it_title = str(it["title"]).strip() if it else ""
        en_title = str(en["title"]).strip() if en else ""
        if en_title and it_title and en_title != it_title:
            title = en_title
            langs = "en+it"
        else:
            title = it_title or en_title
            langs = "it" if it else "en"
        raw_metadata: Dict[str, Any] = {
            "provider": "yahoo_finance_rss",
            "stock_code": code,
            "langs": langs,
            "scraped": True,
        }
        if langs == "en+it":
            raw_metadata["title_en"] = en_title
            raw_metadata["title_it"] = it_title
        elif langs == "it":
            raw_metadata["title_it"] = it_title
        else:
            raw_metadata["title_en"] = en_title
        items.append(
            InformationItem(
                source="yahoo_it",
                source_type="news",
                external_id=key,
                tickers=(code,),
                issuer=code,
                published_at=record["published"],
                title=title,
                document_type="news",
                url=str(record["url"]),
                collected_at=collected_at,
                raw_metadata=raw_metadata,
                market=MARKET_IT,
                summary=record.get("summary"),
                effective_at=record["published"],
            )
        )
    return items
