"""Yahoo Finance CA news connector for market=ca companies."""
...
class YahooCaNewsConnector:
    """Collect Yahoo Finance CA stock news for active CA companies."""

    name = "yahoo_ca"
    provider = "Yahoo Finance CA"
    max_lookback_days = MAX_LOOKBACK_DAYS

    def __init__(
        self,
        client: Optional[YahooCaNewsClient] = None,
        symbol_for: Optional[Callable[[str], str]] = None,
    ) -> None:
        self._client = client or YahooCaNewsClient.from_environment()
    ...
    def collect(self, request: CollectionRequest) -> List[InformationItem]:
        ...
        for ticker in request.tickers:
            ...
            if market != MARKET_CA:
                LOGGER.info(
                    "yahoo_ca ticker=%s market=%s skipped not_ca_market",
                    ticker,
                    market,
                )
                continue
            ...
            try:
                ...
                LOGGER.warning(
                    "yahoo_ca ticker=%s status=failure error=%s",
                    ticker,
                    message,
                )
        self._last_errors = tuple(failures)
        if len(request.tickers) == 1 and failures:
            raise YahooCaNewsRequestError(failures[0][1])
        return items


def _default_symbol_for(ticker: str) -> str:
    """Request-time symbol from the CA universe board (.TO default, .V TSXV)."""
    return ca_yahoo_symbol(ticker)
