"""Yahoo Finance ES news connector for market=es companies."""

from __future__ import annotations

import logging
from datetime import datetime, timezone
from typing import Any, Callable, Dict, List, Mapping, Optional, Tuple

from ....models import CollectionRequest, InformationItem, MARKET_ES
from ....web_repository import normalize_es_ticker
from ..symbols import es_yahoo_symbol
from .client import (
    YahooEsNewsClient,
    YahooEsNewsRequestError,
)

LOGGER = logging.getLogger(__name__)

MAX_LOOKBACK_DAYS = 30


class YahooEsNewsConnector:
    """Collect Yahoo Finance Spain stock news for market=es companies."""

    name = "yahoo_es"
    provider = "Yahoo Finance ES"
    max_lookback_days = MAX_LOOKBACK_DAYS

    def __init__(
        self,
        client: Optional[YahooEsNewsClient] = None,
        symbol_for: Optional[Callable[[str], str]] = None,
    ) -> None:
        self._client = client or YahooEsNewsClient.from_environment()
        self._symbol_for = symbol_for or _default_symbol_for
        self._last_errors: Tuple[Tuple[str, str], ...] = ()

    @property
    def last_errors(self) -> Tuple[Tuple[str, str], ...]:
        return self._last_errors

    def collect(self, request: CollectionRequest) -> List[InformationItem]:
        items: List[InformationItem] = []
        failures: List[Tuple[str, str]] = []
        collected_at = datetime.now(timezone.utc)
        for ticker in request.tickers:
            market = request.market_for(ticker)
            if market != MARKET_ES:
                LOGGER.info(
                    "yahoo_es ticker=%s market=%s skipped not_es_market",
                    ticker,
                    market,
                )
                continue
            code = normalize_es_ticker(ticker)
            symbol = self._symbol_for(code)
            try:
                es_records = self._client.fetch_news(
                    symbol,
                    request.start_date,
                    request.end_date,
                    lang="es-ES",
                )
                en_records = self._client.fetch_news(
                    symbol,
                    request.start_date,
                    request.end_date,
                    lang="en-US",
                )
                items.extend(
                    _map_news(
                        es_records,
                        en_records,
                        code=code,
                        collected_at=collected_at,
                    )
                )
            except Exception as error:
                message = str(error) or error.__class__.__name__
                failures.append((ticker, message))
                LOGGER.warning(
                    "yahoo_es ticker=%s status=failure error=%s",
                    ticker,
                    message,
                )
        self._last_errors = tuple(failures)
        if len(request.tickers) == 1 and failures:
            raise YahooEsNewsRequestError(failures[0][1])
        return items


def _default_symbol_for(ticker: str) -> str:
    """Request-time symbol: canonical ES root plus the .MC suffix."""
    return es_yahoo_symbol(ticker)


def _map_news(
    es_records: List[Mapping[str, Any]],
    en_records: List[Mapping[str, Any]],
    *,
    code: str,
    collected_at: datetime,
) -> List[InformationItem]:
    merged: Dict[str, Dict[str, Optional[Mapping[str, Any]]]] = {}
    for record in es_records:
        merged[str(record["external_id"])] = {"es": record, "en": None}
    for record in en_records:
        key = str(record["external_id"])
        if key in merged:
            merged[key]["en"] = record
        else:
            merged[key] = {"es": None, "en": record}

    items: List[InformationItem] = []
    for key, pair in merged.items():
        es = pair["es"]
        en = pair["en"]
        record = en or es
        if record is None:
            continue
        es_title = str(es["title"]).strip() if es else ""
        en_title = str(en["title"]).strip() if en else ""
        if en_title and es_title and en_title != es_title:
            title = en_title
            langs = "en+es"
        else:
            title = es_title or en_title
            langs = "es" if es else "en"
        raw_metadata: Dict[str, Any] = {
            "provider": "yahoo_finance_rss",
            "stock_code": code,
            "langs": langs,
            "scraped": True,
        }
        if langs == "en+es":
            raw_metadata["title_en"] = en_title
            raw_metadata["title_es"] = es_title
        elif langs == "es":
            raw_metadata["title_es"] = es_title
        else:
            raw_metadata["title_en"] = en_title
        items.append(
            InformationItem(
                source="yahoo_es",
                source_type="news",
                external_id=key,
                tickers=(code,),
                issuer=code,
                published_at=record["published"],
                title=title,
                document_type="news",
                url=str(record["url"]),
                collected_at=collected_at,
                raw_metadata=raw_metadata,
                market=MARKET_ES,
                summary=record.get("summary"),
                effective_at=record["published"],
            )
        )
    return items
