"""Google News (AT) connector for market=at companies."""
...
class GoogleAtNewsConnector:
    """Collect Google News RSS items for market=at companies."""

    name = "google_news_at"
    provider = "Google News (AT)"
    max_lookback_days = MAX_LOOKBACK_DAYS

    def __init__(
        self,
        client: Optional[GoogleAtNewsClient] = None,
        symbol_for: Optional[Callable[[str], str]] = None,
    ) -> None:
        self._client = client or GoogleAtNewsClient.from_environment()
    ...
    def collect(self, request: CollectionRequest) -> List[InformationItem]:
        ...
        for ticker in request.tickers:
            ...
            try:
                ...
                LOGGER.warning(
                    "google_news_at ticker=%s status=failure error=%s",
                    ticker,
                    message,
                )
        self._last_errors = tuple(failures)
        if len(request.tickers) == 1 and failures:
            raise GoogleAtNewsRequestError(failures[0][1])
        return items
