"""Google News (FR) connector for market=fr companies."""
...
class GoogleFrNewsConnector:
    """Collect Google News RSS items for market=fr companies."""
    ...
    def __init__(
        self,
        client: Optional[GoogleFrNewsClient] = None,
        symbol_for: Optional[Callable[[str], str]] = None,
    ) -> None:
        self._client = client or GoogleFrNewsClient.from_environment()
    ...
    def collect(self, request: CollectionRequest) -> List[InformationItem]:
        ...
        for ticker in request.tickers:
            ...
            if market != MARKET_FR:
                LOGGER.info(
                    "google_news_fr ticker=%s market=%s skipped not_fr_market",
                    ticker,
                    market,
                )
                continue
            ...
            try:
                ...
                LOGGER.warning(
                    "google_news_fr ticker=%s status=failure error=%s",
                    ticker,
                    message,
                )
        self._last_errors = tuple(failures)
        if len(request.tickers) == 1 and failures:
            raise GoogleFrNewsRequestError(failures[0][1])
        return items
