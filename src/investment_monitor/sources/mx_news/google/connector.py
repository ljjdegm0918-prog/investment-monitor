"""Google News (MX) connector for market=mx companies."""
...
class GoogleMxNewsConnector:
    """Collect Google News RSS items for market=mx companies."""

    name = "google_news_mx"
    provider = "Google News (MX)"
    max_lookback_days = MAX_LOOKBACK_DAYS

    def __init__(
        self,
        client: Optional[GoogleMxNewsClient] = None,
        symbol_for: Optional[Callable[[str], str]] = None,
    ) -> None:
        self._client = client or GoogleMxNewsClient.from_environment()
    ...
    def collect(self, request: CollectionRequest) -> List[InformationItem]:
        ...
        for ticker in request.tickers:
            ...
            try:
                ...
                LOGGER.warning(
                    "google_news_mx ticker=%s status=failure error=%s",
                    ticker,
                    message,
                )
        self._last_errors = tuple(failures)
        if len(request.tickers) == 1 and failures:
            raise GoogleMxNewsRequestError(failures[0][1])
        return items
