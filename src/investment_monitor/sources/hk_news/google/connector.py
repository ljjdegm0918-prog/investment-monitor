"""Google News (HK) connector for market=hk companies."""
...
class GoogleHkNewsConnector:
    """Collect Google News RSS items for market=hk companies."""

    name = "google_news_hk"
    provider = "Google News (HK)"
    max_lookback_days = MAX_LOOKBACK_DAYS

    def __init__(
        self,
        client: Optional[GoogleHkNewsClient] = None,
        symbol_for: Optional[Callable[[str], str]] = None,
    ) -> None:
        self._client = client or GoogleHkNewsClient.from_environment()
    ...
    def collect(self, request: CollectionRequest) -> List[InformationItem]:
        ...
        for ticker in request.tickers:
            ...
            try:
                ...
                LOGGER.warning(
                    "google_news_hk ticker=%s status=failure error=%s",
                    ticker,
                    message,
                )
        self._last_errors = tuple(failures)
        if len(request.tickers) == 1 and failures:
            raise GoogleHkNewsRequestError(failures[0][1])
        return items
...
def _default_symbol_for(ticker: str) -> str:
    ...
    return code + ".HK" if code else code
