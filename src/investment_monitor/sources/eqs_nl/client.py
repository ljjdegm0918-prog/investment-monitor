"""EQS News (ex-DGAP) key-free JSON client for Dutch issuer disclosures.

Recon (verified live 2026-08-10):
``GET https://www.eqs-news.com/wp-json/eqsnews/v1/news?isin=NL0000235190``
returns JSON ``{status, records:[...]}`` with headline, categoryCode,
dateUtc, companyName and isin — no API key. Coverage of Dutch ISINs is
partial (Airbus SE returns records; ASML/Philips/ING return empty lists),
which is honest rather than a failure. This is the same unofficial public
WordPress JSON surface as the DE EQS connector and may change without
notice; it is NOT an AFM official feed.
"""
...
class EqsNlError(Exception):
    """Base error for EQS News NL collection."""


class EqsNlRequestError(EqsNlError):
    """Raised when the EQS request cannot be completed."""


class EqsNlDataError(EqsNlError):
    """Raised when EQS returns unexpected JSON."""


class EqsNlClient:
    """Small stdlib JSON client for EQS News Dutch issuer disclosures."""
    ...
    def __init__(
        ...
        if not base_url.strip():
            raise ValueError("EQS NL base URL must not be empty.")
        if timeout <= 0:
            raise ValueError("EQS NL timeout must be greater than zero.")
        if max_retries < 0:
            raise ValueError("EQS NL max_retries must not be negative.")
        if requests_per_second <= 0:
            raise ValueError(
                "EQS NL requests_per_second must be greater than zero."
            )
    ...
    @classmethod
    def from_environment(cls) -> "EqsNlClient":
        return cls(
            base_url=os.environ.get("EQS_NL_URL", DEFAULT_BASE_URL),
            public_base=os.environ.get(
                "EQS_NL_PUBLIC_BASE", DEFAULT_PUBLIC_BASE
            ),
            timeout=_env_float("EQS_NL_TIMEOUT_SECONDS", 20.0),
            max_retries=_env_int("EQS_NL_MAX_RETRIES", 1),
            requests_per_second=_env_float(
                "EQS_NL_REQUESTS_PER_SECOND", 1.0
            ),
        )
    ...
    def fetch_by_isin(
        ...
        *,
        ...
        """Fetch EQS news records for an ISIN and filter to the Amsterdam day window."""
        ...
        for page in range(1, max_pages + 1):
            ...
            for raw in page_records:
                ...
                if parsed is None:
                    continue
                day = amsterdam_day(parsed["published_at"])
    ...
    def _get_json(self, url: str) -> Mapping[str, Any]:
        ...
        try:
            ...
            raise EqsNlDataError(
                f"EQS NL returned non-JSON payload for {url}"
            ) from error
        if not isinstance(payload, Mapping):
            raise EqsNlDataError(
                f"EQS NL JSON root must be an object for {url}"
            )
        return payload
        ...
        for attempt in range(self._max_retries + 1):
            ...
            try:
                ...
                if (
                    ...
                    raise EqsNlRequestError(
                        f"EQS NL request failed with HTTP "
                        f"{error.code}: {url}"
                    ) from error
            except URLError as error:
                if attempt == self._max_retries:
                    raise EqsNlRequestError(
                        f"EQS NL request failed after "
                        f"{self._max_retries + 1} attempts: {url}"
                    ) from error
            except TimeoutError as error:
                if attempt == self._max_retries:
                    raise EqsNlRequestError(
                        f"EQS NL request timed out after "
                        f"{self._max_retries + 1} attempts: {url}"
                    ) from error
            self._sleeper(0.5 * (2**attempt))
        raise EqsNlRequestError(f"EQS NL request failed: {url}")
...
def amsterdam_day(value: datetime) -> date:
    """Calendar day in Europe/Amsterdam for an aware/naive UTC datetime."""
    if value.tzinfo is None:
        value = value.replace(tzinfo=timezone.utc)
    return value.astimezone(AMSTERDAM).date()
