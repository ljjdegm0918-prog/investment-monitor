...
| (news) | — | — | Market skeleton plus disclosure/universe boundaries so far: `market=se` companies use canonical root tickers (`ERIC-B` / `ERIC-B.ST` / `eric-b.sto` all store as `ERIC-B`; exchange suffixes `.ST` / `.STO` / `.OMX` / `-ST` etc. are stripped at add time while share-class suffixes like `-B` / `-A` are preserved, Swedish ISINs are kept as-is) and remain unmapped. SE news connectors are planned but not wired yet (SE-3 … SE-5). Finnhub is **US only** and never queried for SE. |
