...
| (news) | — | — | Market skeleton plus disclosure and universe stub so far: `market=ch` companies use canonical root tickers (`NESN` / `NESN.SW` / `NESN-SWX` all store as `NESN`; exchange suffixes `.SW` / `.SWX` / `.S` are stripped at add time, Swiss ISINs are kept as-is) and remain unmapped. CH news connectors are planned but not wired yet (CH-3 … CH-5). Finnhub is **US only** and never queried for CH. |
