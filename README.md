...
| yahoo_ca | news | none | Yahoo Finance CA public RSS (`region=CA`, `lang=en-CA`); `.TO` at request time (`.V` for TSXV boards from the universe cache, otherwise `.TO`); TSXV symbols may return an empty feed; may be loosely related and break without notice |
| google_news_ca | news | none | Key-free Google News RSS search (`q={symbol}`, `hl=en-CA&gl=CA&ceid=CA:en`); may be loosely related and break without notice |
| (disclosure none) | — | — | Regulatory disclosure is **deliberately not wired** (CA-1 spike, re-verified CA-4 on 2026-08-08): SEDAR+ has no official public API; its real search endpoints return Radware 403 to stdlib HTTP clients (non-API paths only return a JavaScript SPA shell), the CSE public API fails the TLS handshake from the current network, and NEO returns Cloudflare 522 origin timeouts. If a stable free JSON disclosure feed appears later, a future slice can wire it. |
...
| Source | Type | Key | Boundaries |
|---|---|---|---|
| asx_announcements | filings | none | ASX company announcements via the ASX site research API (`asx.api.markitdigital.com/asx-research/1.0/companies/{CODE}/announcements`; key-free, live verified 2026-08-08). Undocumented internal JSON — may change without notice; returns only the latest 5 announcements per company (no pagination), so hyper-active companies may be partial; items carry no deep document URL, so `url` points at the per-company announcement list and `document_key` is stored in metadata. AU-4 re-verified (2026-08-08): still latest 5, no pagination/deep link, and no stable key-free second disclosure source was found (no ASX announcements RSS; ASIC has no keyless disclosure JSON). |
| au_universe | breadth cache | none | ASX company directory via the key-free ASX site research API (`/asx-research/1.0/companies/directory`; live 2026-08-08: ~1,840 listed companies); not an IBKR-complete universe; never enters the feed. |
| yahoo_au | news | none | Yahoo Finance AU public RSS (`region=AU`, `lang=en-AU`); `.AX` at request time; may be loosely related and break without notice |
| google_news_au | news | none | Key-free Google News RSS search (`q={symbol}`, `hl=en-AU&gl=AU&ceid=AU:en`); may be loosely related and break without notice |
